...
struct ReservationForm: View {
    ...
    private func validateForm() {
        ...
        let phoneIsValid = isValid(phoneNumber: customerPhoneNumber)
        
        guard nameIsValid && emailIsValid && phoneIsValid
        else {
            ...
            if customerPhoneNumber.isEmpty || !isValid(phoneNumber: customerPhoneNumber) {
                invalidPhoneMessage = "The phone number is invalid and cannot be blank.\n\n"
            }
    ...
    func isValid(phoneNumber: String) -> Bool {
        guard !phoneNumber.isEmpty, phoneNumber.count == 9
        else { return false }
